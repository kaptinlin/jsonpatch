...
func TestDefined_Basic(t *testing.T) {
	...
	if err != nil {
		require.FailNow(t, fmt.Sprintf("Defined.Test(doc, /foo) failed: %v", err))
	}
	if !ok {
	...
	ok, err = definedOp.Test(doc)
	if err != nil {
		require.FailNow(t, fmt.Sprintf("Defined.Test(doc, /qux) failed: %v", err))
	}
	if ok {
	...
	ok, err = definedOp.Test(doc)
	if err != nil {
		require.FailNow(t, fmt.Sprintf("Defined.Test(doc, /baz/qux) failed: %v", err))
	}
	if !ok {
...
func TestDefined_Apply(t *testing.T) {
	...
	if err != nil {
		require.FailNow(t, fmt.Sprintf("Defined.Apply(doc, /foo) failed: %v", err))
	}
...
func TestDefined_Validate(t *testing.T) {
	...
	if err := definedOp.Validate(); err != nil {
		assert.Fail(t, fmt.Sprintf("Validate() = %v, want nil for empty path (root)", err))
	}
}
