package main

import (
	"io"
	"os"
	"strings"
	"testing"
)

func TestMainShowsExpectedOutput(t *testing.T) {
	// This test redirects process-wide stdout.
	output := captureOutput(t, main)
	assertOutputContains(t, output, "Welcome! Hello world! How are you?")
	assertOutputContains(t, output, "My First Document")
	assertOutputContains(t, output, "go,json,patch")
}

func captureOutput(t *testing.T, fn func()) string {
	t.Helper()

	old := os.Stdout
	r, w, err := os.Pipe()
	if err != nil {
		t.Fatal(err)
	}
	os.Stdout = w
	defer func() { os.Stdout = old }()

	fn()

	if err := w.Close(); err != nil {
		t.Fatal(err)
	}
	output, err := io.ReadAll(r)
	if err != nil {
		t.Fatal(err)
	}
	if err := r.Close(); err != nil {
		t.Fatal(err)
	}
	return string(output)
}

func assertOutputContains(t *testing.T, output, want string) {
	t.Helper()

	if !strings.Contains(output, want) {
		t.Fatalf("output does not contain %q:\n%s", want, output)
	}
}
